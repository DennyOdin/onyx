...
function ConnectorRow({
  ...
  const [fullCCPair, setFullCCPair] = useState<CCPairFullInfo | null>(null);
  ...
  const stopRowNavigation = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
  };

  const refreshStatusTable = useCallback(async () => {
    await mutate(
      (key: unknown) => Array.isArray(key) && key[0] === INDEXING_STATUS_URL,
      undefined,
      { revalidate: true }
    );
  }, [mutate]);

  const loadFullCCPair = useCallback(async () => {
    if (fullCCPair) return fullCCPair;

    setIsLoadingFullInfo(true);
    try {
      const data = (await errorHandlingFetcher(
        buildCCPairInfoUrl(ccPairsIndexingStatus.cc_pair_id)
      )) as CCPairFullInfo;

      if (!data?.connector?.id || !data?.credential?.id) {
        toast.error(
          "Loaded cc-pair, but connector_id / credential_id missing in response"
        );
        return null;
      }
      ...
      console.error("Failed to load full connector info:", error);
      toast.error("Failed to load connector details");
      return null;
    } finally {
      setIsLoadingFullInfo(false);
    }
  }, [fullCCPair, ccPairsIndexingStatus.cc_pair_id]);
  ...
  const performStatusUpdate = async (
    ...
    try {
      await setCCPairStatus(ccPairsIndexingStatus.cc_pair_id, newStatus);
  ...
  const handleStatusUpdate = async (
    ...
    if (
      ...
      setShowInvalidConnectorConfirmModal(true);
      return;
    }
  ...
  const handleOpenReIndex = async () => {
    const ccPair = await loadFullCCPair();
    if (!ccPair) return;
    setIsReIndexModalOpen(true);
  };

  const runReIndex = async (fromBeginning: boolean) => {
    const ccPair = await loadFullCCPair();
    if (!ccPair) return;
  ...
  const scheduleConnectorDeletion = async () => {
  ...
  const isReIndexDisabled =
    ccPairsIndexingStatus.in_progress ||
    ccPairsIndexingStatus.cc_pair_status ===
      ConnectorCredentialPairStatus.PAUSED ||
    ccPairsIndexingStatus.cc_pair_status ===
      ConnectorCredentialPairStatus.INVALID;

  const reIndexTooltip = ccPairsIndexingStatus.in_progress
  ...
  const nextStatus = statusIsNotCurrentlyActive(
    ccPairsIndexingStatus.cc_pair_status
  )
  ...
  return (
    <>
            ...
            <DropdownMenu>
              ...
              >
                <DropdownMenuItemWithTooltip
                  ...
                  disabled={isReIndexDisabled || isLoadingFullInfo}
                ...
                {!isDeleting && (
                  <DropdownMenuItemWithTooltip
                    ...
                    className="flex items-center gap-x-2 cursor-pointer px-3 py-2"
                    tooltip={
                      isUpdatingStatus ? "Status update in progress" : undefined
                    }
                  >
                    {statusIsNotCurrentlyActive(
                      ccPairsIndexingStatus.cc_pair_status
      ...
      {isReIndexModalOpen && fullCCPair && (
        <ReIndexModal
          hide={() => setIsReIndexModalOpen(false)}
          onRunIndex={runReIndex}
        />
      )}
      ...
      {showDeleteConnectorConfirmModal && (
        <ConfirmEntityModal
          ...
          entityName={ccPairsIndexingStatus.name || "Unnamed Connector"}
          onClose={() => setShowDeleteConnectorConfirmModal(false)}
          onSubmit={() => void scheduleConnectorDeletion()}
        />
      )}
      ...
      {showInvalidConnectorConfirmModal && (
        <ConfirmEntityModal
          ...
          entityName={ccPairsIndexingStatus.name || "Unnamed Connector"}
          onClose={() => setShowInvalidConnectorConfirmModal(false)}
          onSubmit={() => {
            setShowInvalidConnectorConfirmModal(false);
            void performStatusUpdate(ConnectorCredentialPairStatus.ACTIVE);
          }}
